...
class Model:
    ...
    def print_solution(self):
        if self.solution is not None:
            print(
                "The tableau has an optimal solution x = {}\nThe optimal value is z = {}".format(
                    self.solution, self.z
                )
            )
        elif self.unbounded:
    ...
    def gomory_cut(self, verbose=0):
        ...
        # add the new row
        tableau = np.vstack((tableau, np.array([0.0] * n)))
        # add the new column
        ...
        while optimal == False and unbounded == False:
            ...
            if all(c >= 0 for c in costs):
                ...
                if all(tableau[i, h] < 0 for i in range(m)):
                    ...
                    if verbose == 2:
                        print(
                            "Pivot operation on element {} in the cell: {}\n".format(
                                tableau[t][h], (t, h)
                            )
                        )
            ...
            if all(b >= 0 for b in b_vector):
                ...
                if all(tableau[t][i] >= 0 for i in range(1, n)):
                    ...
                    for i in range(1, n):
                        if (
                                tableau[t][i] < 0
                                and tableau[0][i] / abs(tableau[t][i]) < min
                        ):
